...
/// </summary>
public class RequiredIfNotNullOrEmptySchemaFilter : ISchemaFilter
{
    /// <summary>
    ...
    public void Apply(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        foreach (PropertyInfo property in context.Type.GetProperties())
        {
            ...
            List<RequiredIfNotNullOrEmptyAttribute> attributes =
                property.GetCustomAttributes<RequiredIfNotNullOrEmptyAttribute>(false).ToList();

            // Iterate over the attributes and update the required field
            foreach (RequiredIfNotNullOrEmptyAttribute attribute in attributes)
            {
                // Localize the target
