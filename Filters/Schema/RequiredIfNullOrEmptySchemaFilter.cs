...
/// </summary>
public class RequiredIfNullOrEmptySchemaFilter : ISchemaFilter
{
    /// <summary>
    ...
    public void Apply(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        foreach (PropertyInfo property in context.Type.GetProperties())
        {
            ...
            List<RequiredIfNullOrEmptyAttribute> attributes =
                property.GetCustomAttributes<RequiredIfNullOrEmptyAttribute>(false).ToList();

            // Iterate over the attributes and update the required field
            foreach (RequiredIfNullOrEmptyAttribute attribute in attributes)
            {
                // Localize the target
