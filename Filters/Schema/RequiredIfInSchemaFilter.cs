...
/// </summary>
public class RequiredIfInSchemaFilter : ISchemaFilter
{
    /// <summary>
    ...
    public void Apply(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        foreach (PropertyInfo property in context.Type.GetProperties())
        {
            ...
            List<RequiredIfInAttribute> attributes =
                property.GetCustomAttributes<RequiredIfInAttribute>(false).ToList();

            // Iterate over the attributes and update the required field
            foreach (RequiredIfInAttribute attribute in attributes)
            {
                // Localize the target
