...
public class RequiredIfInAttribute : ValidationAttribute
{
    /// <summary>
    ...
    /// </summary>
    public readonly List<object> MatchValues;
    ...
    public RequiredIfInAttribute(string propertyName, List<object> matchValues, string description = null,
    ...
    /// <param name="propertyName">The property whose value must be analyzed for requirement</param>
    /// <param name="matchValues">The desired values to match for requirement</param>
    public RequiredIfInAttribute(string propertyName, params object[] matchValues)
    {
        // Set the description into the instance
        Description = null;
    ...
    protected override ValidationResult IsValid(object value, ValidationContext validationContext)
    {
        ...
        if (MatchValues.Contains(property.GetValue(instance)) &&
            (value == null || string.IsNullOrEmpty(value.ToString())))
            return new(GenerateErrorMessage(validationContext.MemberName ?? property.Name));
    ...
    public string GenerateDescription(string propertyName, string targetName = null)
    {
        ...
        string matchValues = string.Join(',', MatchValues.Select(v => v.ToString()).ToList());

        // We're done, return the description
        return Description ??
               $"\n * `{propertyName}` is **required** when `{targetName ?? PropertyName}` contains `{matchValues}`";
    }
    ...
    public string GenerateErrorMessage(string propertyName)
    {
        ...
        // We're done, return the description
        return Description ?? $"{propertyName}  is required when {PropertyName} contains {matchValues}";
    }
}
